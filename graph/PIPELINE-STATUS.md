...
Merge commands: python tools/merge.py init|validate|concepts|themes|edges|superthemes|superedges|tissue-themes|stories|paper-overlay|walks|pages [--aliases f]
...
PIPELINE COMPLETE (2026-07-11; stage 14 added 2026-07-12). To add a paper later: rerun stage 1 for it, then rebuild lens layers over the enlarged pool and rerun subsequent stages (see spec). To add another story: append to staging/stage14-stories.json (or a new staging file) and rerun `merge.py stories`.
